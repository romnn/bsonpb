...
func (o UnmarshalOptions) unmarshal(b []byte, m proto.Message) error {
	...
	dec := decoder{bsonDec, o}
	var inputValue bson.M
	if err := dec.Decode(&inputValue); err != nil {
		return err
	}
	
	if err := dec.unmarshalMessage(inputValue, m.ProtoReflect(), false); err != nil {
		return err
	}
...
func (d decoder) newError(pos int, f string, x ...interface{}) error {
	...
	return fmt.Errorf("%s, %v", head+f, x)
}
...
func (d decoder) unmarshalMessage(doc bson.M, m pref.Message, skipTypeURL bool) error {
		...
		*/
		...
		if strings.HasPrefix(name, "[") && strings.HasSuffix(name, "]") {
			...
			if err != nil && err != protoregistry.NotFound {
				return fmt.Errorf("unable to resolve %s: %v", tok.RawString(), err)
			}
			if extType != nil {
		...
		if fd == nil {
			...
			return fmt.Errorf("unknown field %v", tok.RawString())
		}

		// Do not allow duplicate fields.
		num := uint64(fd.Number())
		if seenNums.Has(num) {
			return fmt.Errorf("duplicate field %v", tok.RawString())
		}
		seenNums.Set(num)
		...
		default:
			...
			if od := fd.ContainingOneof(); od != nil {
				...
				if seenOneofs.Has(idx) {
					return fmt.Errorf("error parsing %s, oneof %v is already set", tok.RawString(), od.FullName())
				}
				seenOneofs.Set(idx)
			}
...
func (d decoder) unmarshalScalar(fd pref.FieldDescriptor) (pref.Value, error) {
	...
	return pref.Value{}, fmt.Errorf("invalid value for %v type: %v", kind, tok.RawString())
}
...
Loop:
	for {
		...
		if mmap.Has(pkey) {
			return fmt.Errorf("duplicate map key %v", tok.RawString())
		}
...
func (d decoder) unmarshalMapKey(tok json.Token, fd pref.FieldDescriptor) (pref.MapKey, error) {
	...
	return pref.MapKey{}, fmt.Errorf("invalid value for %v key: %s", kind, tok.RawString())
}